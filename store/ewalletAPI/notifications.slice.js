...
export const api_v1_notification_list = createAsyncThunk("notifications/api_v1_notification_list", async payload => {
  const response = await apiService.api_v1_notification_list(payload);
  return response.data;
});
export const api_v1_notification_create = createAsyncThunk("notifications/api_v1_notification_create", async payload => {
  const response = await apiService.api_v1_notification_create(payload);
  return response.data;
});
export const api_v1_notification_retrieve = createAsyncThunk("notifications/api_v1_notification_retrieve", async payload => {
  const response = await apiService.api_v1_notification_retrieve(payload);
  return response.data;
});
export const api_v1_notification_update = createAsyncThunk("notifications/api_v1_notification_update", async payload => {
  const response = await apiService.api_v1_notification_update(payload);
  return response.data;
});
export const api_v1_notification_partial_update = createAsyncThunk("notifications/api_v1_notification_partial_update", async payload => {
  const response = await apiService.api_v1_notification_partial_update(payload);
  return response.data;
});
export const api_v1_notification_destroy = createAsyncThunk("notifications/api_v1_notification_destroy", async payload => {
...
const notificationsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(api_v1_notification_list.pending, (state, action) => {
    ...
    }).addCase(api_v1_notification_partial_update.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.map(record => record.id === action.payload.id ? action.payload : record);
        state.api.loading = "idle";
      }
